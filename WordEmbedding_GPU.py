...
for epoch in range(10) :
    ...
    for context, target in trigrams :
        ...
        context_var = Variable(torch.cuda.LongTensor(context_idxs))

        model.zero_grad()
        log_probs = model(context_var)
        loss = loss_function(log_probs, Variable(torch.cuda.LongTensor([D.word_to_ix[target]])))
        loss.backward()
        optimizer.step()
...
for word in D.vocab :
    ...
    word_var = Variable(torch.cuda.LongTensor(word_idx))
